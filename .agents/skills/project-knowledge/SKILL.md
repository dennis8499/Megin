...
日常交付使用 `scripts/run_full_suite.py --scope all --profile local --fixture-root .knowledge-test-tmp/fixtures --jobs <bounded-workers> --evidence-root .knowledge-test-tmp/evidence --ready-payload-sha256 <ready-digest>`。Profile runner為每個parallel-safe command建立獨立fixture、temp與Delivery registry，fail-fast後才循序執行performance scenarios；split command以logical ID及完整child inventory證明覆蓋。Create-only evidence index保存原始stdout／stderr、exit、耗時、worker cleanup、Git input、環境與Ready binding，可用`implementation-execution/scripts/validation_evidence.py verify`重驗。未帶`--profile`的舊命令維持循序相容行為。

Release 使用相同 runner 的 `--profile release`，但跨平台完成證據仍要求 `.github/workflows/knowledge-portability.yml` 的等價 Windows 與 Linux reports；one local OS result is not Windows/Linux evidence。BUILD-FULL 會編譯每個 Git-eligible `*.py`、解析每個 Git-eligible `*.schema.json`，並報告精確 `python_files_checked` 與 `json_schema_files_checked` inventory；partial skill-local scan 不足。Machine shapes 定義於 `schemas/knowledge-contracts.schema.json`。
