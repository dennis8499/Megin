...
REVIEW_FILE_ROLES = {"supporting", "automatic"}
REVIEW_PROJECTION = "gate-specific-summary/v1"
BUG_ASSESSMENT_PATH_RE = re.compile(
    r"^docs/bugs/(bug-[a-z0-9]+(?:-[a-z0-9]+)*)/assessment-([1-9][0-9]*)\.(md|json)$"
)
IMPLEMENTATION_OUTCOME_PATH_RE = re.compile(
    r"^docs/work/[a-z0-9]+(?:-[a-z0-9]+)*/implementation/"
    r"outcome(?:-[1-9][0-9]*)?\.json$"
)
OCCURRENCE_MAP_PATH_RE = re.compile(r"(?:^|/)occurrence_map\.(?:json|ya?ml)$")
GATE_PURPOSES = {
    "bootstrap": (
        "Review the bootstrap knowledge classification and proposed repository "
        "knowledge files before approval."
    ),
    "bootstrap-quarantine": (
        "Review the unresolved legacy classification and proposed quarantine "
        "knowledge files before approval."
    ),
    "requirements": (
        "Review the Requirements artifact, optional BUG assessment, and proposed "
        "knowledge changes before approval."
    ),
    "planning": (
        "Review the complete Ready-plan bundle, occurrence classification, and "
    ...
    "lint-repair": (
        "Review the bound lint findings and proposed knowledge repairs before "
        "approval."
    ),
}
GATE_CHANGE_DESCRIPTIONS = {
    "bootstrap": "Publish classified legacy knowledge through the bootstrap Candidate.",
    "bootstrap-quarantine": (
        "Quarantine unresolved legacy knowledge through the bootstrap Candidate."
    ),
    "requirements": (
        "Create the formal Requirements bundle and its proposed-knowledge records."
    ),
    "planning": "Create the Ready-plan bundle and its proposed-knowledge records.",
    "implementation": "Publish an implementation Outcome-derived knowledge revision.",
    "bug": "Publish a BUG Outcome-derived incident knowledge revision.",
    "lint-repair": "Apply only repairs derived from the bound knowledge lint result.",
}
STANDALONE_IMPLEMENTATION_PURPOSE = (
    "Review the standalone Knowledge Candidate and proposed observed knowledge "
    "changes before approval."
)
STANDALONE_IMPLEMENTATION_CHANGE_DESCRIPTION = (
    "Publish the standalone Candidate's proposed observed knowledge revision."
)
_BUG_ASSESSMENT_RUNTIME: tuple[Any, dict[str, Any]] | None = None
...
def _candidate_payload_sha256(candidate: dict[str, Any]) -> str:
    ...
    for review_file in normalized.get("review_files", []):
        if isinstance(review_file, dict) and review_file.get("role") == "automatic":
            # Automatic evidence can contain the Candidate's own stable identity.
            # Its bytes are bound by review.json; canonical sentinels avoid a hash cycle.
            review_file["sha256"] = FINALIZER_HASH_SENTINEL
            review_file["byte_count"] = 0
...
def _formal_target_allowed(relative: str, stage: str | None, work_id: str | None) -> bool:
    ...
    if stage == "requirements":
        return (
            re.fullmatch(rf"{root}/requirements(?:-[2-9][0-9]*)?\.md", relative)
            is not None
            or BUG_ASSESSMENT_PATH_RE.fullmatch(relative) is not None
        )
    if stage == "planning":
        return re.fullmatch(
            rf"{root}/plan(?:-[2-9][0-9]*)?/[^/]+\.(?:md|json|yaml|yml)",
            relative,
        ) is not None
    return False


def _bug_assessment_contract() -> tuple[Any, dict[str, Any]]:
    """Load the BUG owner's dependency-free validator without importing its package."""

    global _BUG_ASSESSMENT_RUNTIME
    if _BUG_ASSESSMENT_RUNTIME is not None:
        return _BUG_ASSESSMENT_RUNTIME
    skills_root = Path(__file__).resolve().parents[2]
    script = skills_root / "bug-diagnosis" / "scripts" / "validate_contracts.py"
    schema_path = skills_root / "bug-diagnosis" / "references" / "assessment.schema.json"
    spec = importlib.util.spec_from_file_location(
        "knowledge_bug_assessment_validator",
        script,
    )
    if spec is None or spec.loader is None:
        raise KnowledgeError(
            "BUG_ASSESSMENT_INVALID",
            "BUG assessment validator is unavailable",
            exit_code=2,
        )
    ...
    try:
        ...
        raise KnowledgeError(
            "BUG_ASSESSMENT_INVALID",
            "BUG assessment contract cannot be loaded",
            exit_code=2,
        ) from exc
    _BUG_ASSESSMENT_RUNTIME = (module, schema)
    return _BUG_ASSESSMENT_RUNTIME


def _validated_bug_assessment_pair(
    *,
    sidecar_path: str,
    sidecar_bytes: bytes,
    markdown_path: str,
    ...
    require_create_only: bool = False,
) -> dict[str, Any]:
    """Validate prospective assessment bytes without requiring repository materialization."""

    normalized_sidecar = normalized_path(sidecar_path)
    normalized_markdown = normalized_path(markdown_path)
    sidecar_match = BUG_ASSESSMENT_PATH_RE.fullmatch(normalized_sidecar)
    markdown_match = BUG_ASSESSMENT_PATH_RE.fullmatch(normalized_markdown)
    if (
        sidecar_match is None
        or markdown_match is None
        or sidecar_match.group(3) != "json"
        or markdown_match.group(3) != "md"
        or sidecar_match.groups()[:2] != markdown_match.groups()[:2]
    ):
        raise KnowledgeError(
            "BUG_ASSESSMENT_INVALID",
    ...
    try:
        record = _strict_object(sidecar_bytes, "BUG assessment sidecar")
    except KnowledgeError as exc:
        raise KnowledgeError(
            "BUG_ASSESSMENT_INVALID",
            "BUG assessment sidecar is not strict UTF-8 JSON",
            exit_code=2,
        ) from exc
    validator, schema = _bug_assessment_contract()
    errors = list(
    ...
    if record.get("bug_id") != sidecar_match.group(1):
        errors.append("assessment bug_id differs from its canonical path")
    if record.get("revision") != int(sidecar_match.group(2)):
        errors.append("assessment revision differs from its canonical path")
    markdown = record.get("markdown")
    if not isinstance(markdown, dict) or markdown.get("path") != normalized_markdown:
        errors.append("assessment Markdown binding differs from its canonical path")
    elif markdown.get("sha256") != sha256_bytes(markdown_bytes):
        errors.append("assessment Markdown binding differs from its Candidate bytes")
    if record.get("verdict") not in {"confirmed", "likely"}:
        errors.append("Requirements BUG assessment verdict must be confirmed or likely")
    if record.get("disposition") != "delivery":
        errors.append("Requirements BUG assessment disposition must be delivery")
    if require_create_only:
        if repository_root is None:
            errors.append("create-only validation requires a repository root")
        else:
            errors.extend(
                validator.create_only_errors(
                    repository_root,
                    sidecar_match.group(1),
    ...
    if errors:
        raise KnowledgeError(
            "BUG_ASSESSMENT_INVALID",
            f"BUG assessment Candidate violates its owner contract ({len(errors)} issue(s))",
            exit_code=2,
            evidence_refs=errors[:5],
        )
    return record
...
def _read_review_file(candidate_path: Path, review_file: dict[str, Any]) -> bytes:
    relative = normalized_path(str(review_file.get("stored_path", "")))
    if not relative.startswith("review-files/"):
        raise KnowledgeError(
            "REVIEW_FILE_INVALID",
            "sealed review file is outside review-files/",
            exit_code=3,
        )
    root = candidate_path.parent
    ...
    try:
        ...
        raise KnowledgeError(
            "REVIEW_FILE_INVALID",
            "sealed review file escapes Candidate storage",
            exit_code=3,
        ) from exc
    raw = knowledge_governance._stable_registry_read(
        registry_root,
        path,
        missing_code="REVIEW_FILE_MISSING",
        invalid_code="REVIEW_FILE_DRIFT",
        label="sealed review file",
    )
    if (
        len(raw) != review_file.get("byte_count")
        or sha256_bytes(raw) != review_file.get("sha256")
    ):
        raise KnowledgeError(
            "REVIEW_FILE_DRIFT",
            "sealed review file identity drifted",
            exit_code=3,
...
def _review_target_bytes(
    candidate: dict[str, Any],
    postimages: list[tuple[str, bytes]],
    review_files: list[tuple[str, bytes]],
) -> dict[str, bytes]:
    postimage_by_ref = dict(postimages)
    review_file_by_ref = dict(review_files)
    ...
    for operation in candidate["operations"]:
        ...
        if folded in seen_targets:
            raise KnowledgeError(
                ...
                "Candidate manifest targets collide across file roles",
                exit_code=3,
            )
        seen_targets.add(folded)
        result[target_path] = postimage_by_ref[str(operation["postimage_ref"])]
    for descriptor in candidate.get("review_files", []):
        target_path = str(descriptor["target_path"])
        folded = target_path.casefold()
        if folded in seen_targets:
            raise KnowledgeError(
                "REVIEW_TARGET_COLLISION",
        ...
        seen_targets.add(folded)
        result[target_path] = review_file_by_ref[str(descriptor["stored_path"])]
    return result


def _bug_assessment_projection(
    target_bytes: dict[str, bytes],
) -> dict[str, Any] | None:
    matches = [
        (path, raw)
        for path, raw in target_bytes.items()
        if BUG_ASSESSMENT_PATH_RE.fullmatch(path) is not None and path.endswith(".json")
    ]
    if not matches:
        return None
    path, raw = matches[0]
    assessment = _strict_object(raw, f"BUG assessment summary source {path}")
    verdict = str(assessment["verdict"])
    severity = str(assessment["severity"])
    source_relation = str(assessment["source"]["relation"])
    reproduction_status = str(assessment["reproduction"]["status"])
    root_cause_status = str(assessment["root_cause"]["status"])
    disposition = str(assessment["disposition"])
    security_risk = bool(assessment["risk"]["security_privacy_or_data_risk"])
    if security_risk:
        classification_risk = "security-review-required"
    elif severity in {"critical", "high"}:
        classification_risk = "high-impact"
    elif (
        verdict == "insufficient-evidence"
        or reproduction_status != "reproduced"
        or root_cause_status != "confirmed"
    ):
    ...
    return {
        ...
        "security_privacy_or_data_risk": security_risk,
        "classification_risk": classification_risk,
    }


def _occurrence_projection(target_bytes: dict[str, bytes]) -> dict[str, Any] | None:
    matches = [
        (path, raw)
        for path, raw in target_bytes.items()
        if OCCURRENCE_MAP_PATH_RE.search(path) is not None
    ]
    if not matches:
        return None
    path, raw = matches[0]
    if path.endswith(".json"):
        occurrence = _strict_object(raw, f"occurrence map summary source {path}")
        raw_mode = occurrence.get("change_mode")
        manual_review_item_count = sum(
            1
            for collection in (
                occurrence.get("categories", {}).values()
                if isinstance(occurrence.get("categories"), dict)
                else ()
            )
            if isinstance(collection, dict)
            and collection.get("action") == "manual_review"
        ) + sum(
            1
            for item in (
                occurrence.get("exceptions", [])
                if isinstance(occurrence.get("exceptions"), list)
                else ()
            )
            if isinstance(item, dict) and item.get("action") == "manual_review"
        )
    else:
        text = raw.decode("utf-8")
        modes = re.findall(
            r"(?m)^change_mode:\s*([a-z0-9_-]+)\s*(?:#.*)?$",
            text,
        )
        raw_mode = modes[0] if len(modes) == 1 else None
        manual_review_item_count = len(
            re.findall(
                r'''(?mx)
                ^[ \t]*(?:-\s*)?action\s*:\s*
    ...
    change_mode = (
        str(raw_mode)
        if raw_mode in {"bulk_edit", "targeted_edit", "single_edit"}
        else "unknown"
    )
    manual_review_required = (
        change_mode == "bulk_edit" or manual_review_item_count > 0
    )
    classification_risk = (
        "unknown"
        if change_mode == "unknown"
...
def _implementation_outcome_projection(
    target_bytes: dict[str, bytes],
) -> dict[str, Any] | None:
    matches = [
        (path, raw)
        for path, raw in target_bytes.items()
        if IMPLEMENTATION_OUTCOME_PATH_RE.fullmatch(path) is not None
    ]
    if not matches:
        return None
    path, raw = matches[0]
    outcome = _strict_object(raw, f"implementation Outcome summary source {path}")
    if outcome.get("schema") != "implementation-outcome/v1":
        return None
    bug = outcome.get("bug") if isinstance(outcome.get("bug"), dict) else {}
    return {
        "work_kind": str(outcome["work_kind"]),
        "result": str(outcome["result"]),
        "knowledge_decision": str(outcome["knowledge_decision"]),
        "changed_path_count": len(outcome["changes"]),
        "known_deviation_count": len(outcome["known_deviations"]),
        "residual_risk_count": len(bug.get("residual_risks", [])),
        "follow_up_count": len(bug.get("follow_up", [])),
    }


def _classification_projection(candidate: dict[str, Any]) -> dict[str, list[str]]:
    classification = candidate.get("classification")
    keys = ("terminal", "candidate", "conflict", "unknown")
    if not isinstance(classification, dict) or set(classification) != set(keys):
        raise KnowledgeError(
            "CANDIDATE_INVALID",
            "bootstrap Candidate classification is missing or invalid",
            exit_code=3,
        )
    ...
    for key in keys:
        ...
        normalized = [normalized_path(value) for value in values]
        if normalized != values or normalized != sorted(
            normalized,
            key=lambda value: value.encode("utf-8"),
        ):
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "bootstrap Candidate classification is not canonical",
                exit_code=3,
            )
        folded = [value.casefold() for value in normalized]
        if len(set(folded)) != len(folded) or any(value in seen for value in folded):
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "bootstrap Candidate classification contains a path collision",
                exit_code=3,
            )
        seen.update(folded)
        projected[key] = list(normalized)
    return projected


def _lint_projection(target_bytes: dict[str, bytes]) -> dict[str, Any] | None:
    raw = target_bytes.get("automatic/knowledge-lint.json")
    if raw is None:
        return None
    lint = _strict_object(raw, "knowledge lint summary source")
    diagnostics = lint.get("diagnostics")
    outcome = lint.get("outcome")
    if (
        lint.get("schema") != "knowledge-lint/v1"
        or outcome not in {"passed", "failed", "decision_required"}
        ...
        raise KnowledgeError(
            "CANDIDATE_INVALID",
            "automatic lint evidence cannot produce a closed Gate summary",
            exit_code=3,
        )
    return {
        "outcome": outcome,
...
def _gate_specific_projection(
    ...
    target_bytes = _review_target_bytes(candidate, postimages, review_files)
    implementation_outcome = _implementation_outcome_projection(target_bytes)
    purpose = GATE_PURPOSES[stage]
    change_description = GATE_CHANGE_DESCRIPTIONS[stage]
    if stage == "implementation" and implementation_outcome is None:
        purpose = STANDALONE_IMPLEMENTATION_PURPOSE
        change_description = STANDALONE_IMPLEMENTATION_CHANGE_DESCRIPTION
    change = {
        "description": change_description,
        "operation_count": len(operations),
        "create_count": sum(
            1 for operation in operations if operation["kind"] == "create"
        ),
        "update_count": sum(
            1 for operation in operations if operation["kind"] == "update"
        ),
        "review_file_count": len(candidate.get("review_files", [])),
    }
    material = {
        "bug_assessment": _bug_assessment_projection(target_bytes),
        "occurrence": _occurrence_projection(target_bytes),
        "implementation_outcome": implementation_outcome,
    }
    summary_fields: dict[str, Any] = {}
    if stage in {"bootstrap", "bootstrap-quarantine"}:
        summary_fields["classification"] = _classification_projection(candidate)
    if stage == "lint-repair":
        lint = _lint_projection(target_bytes)
        if lint is None:
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "lint-repair Candidate has no bound automatic lint evidence",
                exit_code=3,
            )
        summary_fields["lint"] = lint
    return purpose, change, material, summary_fields
...
def _human_gate_review(
    *,
    ...
    operation_by_ref = {
        str(operation["postimage_ref"]): operation
        for operation in candidate["operations"]
    }
    review_file_by_ref = {
        str(review_file["stored_path"]): review_file
        for review_file in candidate.get("review_files", [])
    }
    manifest = [
        {
            "role": "candidate",
            "target_path": candidate_ref,
            "stored_path": "candidate.json",
        ...
        *[
            {
                "role": "postimage",
                "target_path": str(operation_by_ref[stored_path]["path"]),
                "stored_path": stored_path,
                "sha256": sha256_bytes(postimage),
                "byte_count": len(postimage),
            }
            for stored_path, postimage in postimages
        ],
        *[
            {
                "role": str(review_file_by_ref[stored_path]["role"]),
                "target_path": str(review_file_by_ref[stored_path]["target_path"]),
                "stored_path": stored_path,
                "sha256": sha256_bytes(raw),
                "byte_count": len(raw),
            }
            for stored_path, raw in review_files
        ],
    ]
    automatic_evidence: list[dict[str, Any]] = []
    for stored_path, raw in review_files:
        descriptor = review_file_by_ref[stored_path]
        if descriptor["role"] != "automatic":
            continue
        value = _strict_object(raw, "automatic Gate evidence")
        automatic_evidence.append(
            {
    ...
    review_ref = candidate_ref.removesuffix("candidate.json") + "review.json"
    summary = {
        "purpose": f"Review the complete {candidate['stage']} Candidate before approval.",
        "decision": str(candidate["decision"]),
        "affected_path_count": len(candidate["operations"]),
        "affected_paths": [
            str(operation["path"])
            for operation in candidate["operations"]
        ],
    ...
    risk_and_compatibility = {
        "approval_scope": "Approval authorizes only the existing Candidate apply flow.",
        "automatic_gates": (
            "Automatic validation, review, test, security, lint, schema, "
            "and CI evidence remain unchanged."
        ),
        "history": "Historical Ready artifacts are not rewritten.",
    }
    if candidate.get("review_projection") == REVIEW_PROJECTION:
        purpose, change, material, summary_fields = _gate_specific_projection(
            candidate,
            postimages,
            review_files,
        )
        ...
        risk_and_compatibility["material"] = material
    return _json_bytes(
        {
            "schema": "human-gate-review/v1",
            "gate": {
                "name": str(candidate["stage"]),
                "status": "Candidate",
            },
            "summary": summary,
            "risk_and_compatibility": risk_and_compatibility,
            "validation": {
                "outcome": "passed",
                "checks": [
                    "candidate-digest",
                    "complete-file-manifest",
                    "preimage-and-source-binding",
            ...
            "identity": {
                ...
                "prospective_approval": copy.deepcopy(candidate["approval"]),
            },
...
def _validate_stage_semantics(
    ...
    *,
    ...
    if stage == "requirements":
        ...
        requirement_items = [
            item for item in formal if requirement_pattern.fullmatch(item["relative"])
        ]
        assessment_items = [item for item in formal if item not in requirement_items]
        if len(requirement_items) != 1 or len(assessment_items) not in {0, 2}:
            raise KnowledgeError(
                "FORMAL_ARTIFACT_MISSING",
                "requirements Candidate needs one Requirements file and either no BUG assessment or one Markdown/JSON pair",
                exit_code=2,
            )
        if assessment_items:
            by_suffix = {Path(item["relative"]).suffix: item for item in assessment_items}
            if set(by_suffix) != {".md", ".json"}:
                raise KnowledgeError(
                    "BUG_ASSESSMENT_INVALID",
                    "Requirements BUG assessment must contain one Markdown and one JSON postimage",
                    exit_code=2,
                )
            _validated_bug_assessment_pair(
                sidecar_path=by_suffix[".json"]["relative"],
                sidecar_bytes=by_suffix[".json"]["postimage"],
                markdown_path=by_suffix[".md"]["relative"],
                markdown_bytes=by_suffix[".md"]["postimage"],
...
def seal_candidate_draft(
    ...
    *,
    ...
    known_secret_values: Iterable[str] = (),
    _allow_automatic_evidence: bool = False,
) -> dict[str, Any]:
    ...
    allowed_draft_fields = {*required_draft_fields, "review_files", "classification"}
    if (
        not isinstance(draft, dict)
        or not required_draft_fields.issubset(draft)
        or not set(draft).issubset(allowed_draft_fields)
    ):
        raise KnowledgeError(
            "DRAFT_CONTRACT_INVALID",
    ...
    if stage in {"bootstrap", "bootstrap-quarantine"}:
        expected_classification = knowledge_governance.classify_repository(repo)
        if classification != expected_classification:
            raise KnowledgeError(
                "DRAFT_CONTRACT_INVALID",
                "bootstrap classification must equal the current repository classification",
                exit_code=2,
            )
        classification = copy.deepcopy(expected_classification)
    elif "classification" in draft:
        raise KnowledgeError(
            "DRAFT_CONTRACT_INVALID",
            "classification is reserved for bootstrap Candidates",
            exit_code=2,
        )
    if (
    ...
    draft_review_files = draft.get("review_files", [])
    if not isinstance(draft_review_files, list):
        raise KnowledgeError(
            "DRAFT_CONTRACT_INVALID",
            "review_files must be an array",
            exit_code=2,
        )
    review_file_descriptors: list[dict[str, Any]] = []
    review_files: list[tuple[str, bytes]] = []
    seen_review_targets: set[str] = set()
    seen_review_targets_casefolded: set[str] = set()
    for index, review_file in enumerate(draft_review_files, 1):
        if not isinstance(review_file, dict) or set(review_file) != {
            "role",
            "target_path",
            "content",
        }:
            raise KnowledgeError(
                "DRAFT_CONTRACT_INVALID",
                "draft review file has unknown or missing fields",
                exit_code=2,
            )
        role = review_file.get("role")
        content = review_file.get("content")
        if role not in REVIEW_FILE_ROLES or not isinstance(content, str):
            raise KnowledgeError(
                "DRAFT_CONTRACT_INVALID",
                "draft review file role or content is invalid",
                exit_code=2,
            )
        target_path = normalized_path(str(review_file.get("target_path", "")))
        folded = target_path.casefold()
        if target_path in seen_review_targets or folded in seen_review_targets_casefolded:
            raise KnowledgeError(
                "DRAFT_CONTRACT_INVALID",
                "draft review file target is duplicated or case-colliding",
                exit_code=2,
            )
        seen_review_targets.add(target_path)
        seen_review_targets_casefolded.add(folded)
        raw = content.encode("utf-8")
        if role == "supporting":
            _scan_secrets(raw, known_secret_values, target_path)
            source_path = repo / Path(*target_path.split("/"))
            if (
                target_path not in eligible
                or _redirected(source_path, repo)
                or not source_path.is_file()
                or source_path.read_bytes() != raw
            ):
                raise KnowledgeError(
                    "REVIEW_SOURCE_DRIFT",
                    f"supporting review source is missing or drifted: {target_path}",
                    exit_code=3,
                )
        elif (
            not _allow_automatic_evidence
            or stage != "lint-repair"
            or target_path != "automatic/knowledge-lint.json"
        ):
            raise KnowledgeError(
                "DRAFT_CONTRACT_INVALID",
                "automatic review evidence is reserved for lint-repair",
                exit_code=2,
            )
        else:
            automatic = _strict_object(raw, "automatic knowledge lint")
            if (
                automatic.get("schema") != "knowledge-lint/v1"
                or automatic.get("repair_candidate_ref") is not None
                or automatic.get("repair_candidate_payload_sha256") is not None
            ):
                raise KnowledgeError(
                    "DRAFT_CONTRACT_INVALID",
                    "automatic lint evidence must be an unbound knowledge-lint/v1 result",
                    exit_code=2,
                )
        suffix = Path(target_path).suffix or ".txt"
        stored_path = f"review-files/{index:04d}{suffix}"
        review_file_descriptors.append(
            {
                "role": role,
        ...
        review_files.append((stored_path, raw))
    ...
    for index, operation in enumerate(draft_operations, 1):
        ...
        folded = relative.casefold()
        if folded in seen_review_targets_casefolded:
            raise KnowledgeError(
                "REVIEW_TARGET_COLLISION",
                "operation and review file targets collide under case-insensitive path rules",
                exit_code=3,
            )
        if relative in seen:
    ...
    seed = {
        ...
        "review_files": [
            {
                **copy.deepcopy(review_file),
                "sha256": (
                    FINALIZER_HASH_SENTINEL
                    if review_file["role"] == "automatic"
                    else review_file["sha256"]
                ),
                "byte_count": (
                    0 if review_file["role"] == "automatic" else review_file["byte_count"]
                ),
            }
            for review_file in review_file_descriptors
        ],
    }
    if classification is not None:
        seed["classification"] = copy.deepcopy(classification)
    ...
    candidate = {
        ...
        "source_snapshot": source_snapshot,
        "operations": operations,
        "review_files": review_file_descriptors,
        "payload_sha256": "",
        "status": "Candidate",
    }
    if classification is not None:
        candidate["classification"] = copy.deepcopy(classification)
    candidate["payload_sha256"] = _candidate_payload_sha256(candidate)
    resolved_review_files: list[tuple[str, bytes]] = []
    for descriptor, (stored_path, raw) in zip(review_file_descriptors, review_files):
        if descriptor["role"] == "automatic":
            automatic = _strict_object(raw, "automatic knowledge lint")
            automatic["repair_candidate_ref"] = candidate_ref
            automatic["repair_candidate_payload_sha256"] = candidate["payload_sha256"]
            raw = _json_bytes(automatic)
            descriptor["sha256"] = sha256_bytes(raw)
            descriptor["byte_count"] = len(raw)
        resolved_review_files.append((stored_path, raw))
    review_files = resolved_review_files
    if candidate["payload_sha256"] != _candidate_payload_sha256(candidate):
        raise KnowledgeError(
            "CANDIDATE_DIGEST_DRIFT",
            "automatic review evidence changed the Candidate digest",
            exit_code=3,
        )
    promotion = {
...
def _validated_review_bundle(
    ...
    *,
    ...
    registry_root = repo_root.parents[1]
    review_raw = knowledge_governance._stable_registry_read(
        registry_root,
        review_path,
        missing_code="LEGACY_RESEAL_REQUIRED",
        invalid_code="REVIEW_MANIFEST_DRIFT",
        label="human Gate review",
    )
    actual_review_sha256 = sha256_bytes(review_raw)
    if expected_review_sha256 is not None and expected_review_sha256 != actual_review_sha256:
        raise KnowledgeError(
            "REVIEW_BINDING_DRIFT",
            "approval review digest differs from the current immutable review bundle",
            exit_code=3,
            evidence_refs=[str(review_path)],
            recoverable=True,
        )
    review = _strict_object(review_raw, "human Gate review")
    candidate_raw = knowledge_governance._stable_registry_read(
        registry_root,
        candidate_path,
        missing_code="CANDIDATE_MISSING",
        invalid_code="CANDIDATE_INVALID",
    ...
    operations = candidate.get("operations")
    if not isinstance(operations, list) or len(operations) != len(validated_operations):
        raise KnowledgeError(
            "CANDIDATE_INVALID",
            "Candidate operation count changed during review validation",
            exit_code=3,
        )
    postimages = [
        (str(operation["postimage_ref"]), validated["postimage"])
        for operation, validated in zip(operations, validated_operations)
    ]
    descriptors = candidate.get("review_files", [])
    if not isinstance(descriptors, list):
        raise KnowledgeError(
            "CANDIDATE_INVALID",
            "Candidate review_files is not an array",
            exit_code=3,
        )
    review_files: list[tuple[str, bytes]] = []
    seen_stored: set[str] = set()
    seen_targets = {
        str(operation["relative"]).casefold()
    ...
    for descriptor in descriptors:
        if not isinstance(descriptor, dict) or set(descriptor) != {
            ...
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "Candidate review file descriptor is invalid",
                exit_code=3,
            )
        role = descriptor.get("role")
        target_path = normalized_path(str(descriptor.get("target_path", "")))
        stored_path = normalized_path(str(descriptor.get("stored_path", "")))
        if role not in REVIEW_FILE_ROLES or not stored_path.startswith("review-files/"):
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "Candidate review file role or storage path is invalid",
                exit_code=3,
            )
        if stored_path.casefold() in seen_stored:
            raise KnowledgeError(
                "CANDIDATE_INVALID",
                "Candidate review files contain a storage path collision",
                exit_code=3,
            )
        if target_path.casefold() in seen_targets:
            raise KnowledgeError(
                "REVIEW_TARGET_COLLISION",
                "Candidate manifest targets collide across file roles",
                exit_code=3,
            )
        seen_stored.add(stored_path.casefold())
        seen_targets.add(target_path.casefold())
        raw = _read_review_file(candidate_path, descriptor)
        if role == "supporting":
            _scan_secrets(raw, (), target_path)
        if role == "supporting":
            source_path = repo / Path(*target_path.split("/"))
            if (
                target_path not in eligible
                ...
                raise KnowledgeError(
                    "REVIEW_SOURCE_DRIFT",
                    f"supporting review source changed: {target_path}",
                    exit_code=3,
                    evidence_refs=[target_path],
                    recoverable=True,
                )
        else:
            automatic = _strict_object(raw, "automatic Gate evidence")
            if (
                candidate.get("stage") != "lint-repair"
                or target_path != "automatic/knowledge-lint.json"
                or automatic.get("schema") != "knowledge-lint/v1"
                or automatic.get("repair_candidate_ref") != candidate_ref
                or automatic.get("repair_candidate_payload_sha256")
                != candidate.get("payload_sha256")
            ):
                raise KnowledgeError(
                    "REVIEW_FILE_DRIFT",
                    "automatic Gate evidence differs from its Candidate identity",
                    exit_code=3,
                    evidence_refs=[str(candidate_path.parent / stored_path)],
                    recoverable=True,
                )
        review_files.append((stored_path, raw))
    expected_files = {
        "candidate.json",
        "review.json",
        *(str(operation["postimage_ref"]) for operation in operations),
        *(stored_path for stored_path, _ in review_files),
    }
    actual_files: set[str] = set()
    for path in candidate_path.parent.rglob("*"):
        if knowledge_governance._registry_path_redirected(registry_root, path):
            raise KnowledgeError(
                "REVIEW_MANIFEST_DRIFT",
                "immutable Candidate storage contains a redirected entry",
                exit_code=3,
                evidence_refs=[str(path)],
                recoverable=True,
            )
        if path.is_file():
            actual_files.add(path.relative_to(candidate_path.parent).as_posix())
        elif not path.is_dir():
            raise KnowledgeError(
                "REVIEW_MANIFEST_DRIFT",
                "immutable Candidate storage contains an unsupported entry",
                exit_code=3,
                evidence_refs=[str(path)],
                recoverable=True,
            )
    if actual_files != expected_files:
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "immutable Candidate file inventory differs from the complete review bundle",
            exit_code=3,
            evidence_refs=sorted(actual_files ^ expected_files),
            recoverable=True,
        )
    expected_review = _human_gate_review(
        repo_id=repo_id,
        candidate=candidate,
        candidate_ref=candidate_ref,
    ...
    if review_raw != expected_review:
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "human Gate review no longer matches the complete sealed Candidate",
            exit_code=3,
            evidence_refs=[str(review_path)],
            recoverable=True,
        )
    return review, review_raw, review_path


def _project_review_manifest(
    *,
    registry_root: Path,
    candidate_path: Path,
    review: dict[str, Any],
    ...
    for entry in review["review_bundle"]["manifest"]:
        stored_path = normalized_path(str(entry["stored_path"]))
        direct_path = candidate_path.parent / Path(*stored_path.split("/"))
        try:
            direct_path.relative_to(candidate_path.parent)
        except ValueError as exc:
            raise KnowledgeError(
                "REVIEW_MANIFEST_DRIFT",
                "review manifest file escapes the immutable Candidate bundle",
                exit_code=3,
            ) from exc
        raw = knowledge_governance._stable_registry_read(
            registry_root,
            ...
            missing_code="REVIEW_FILE_MISSING",
            invalid_code="REVIEW_FILE_DRIFT",
            label="human Gate review file",
        )
        if len(raw) != entry["byte_count"] or sha256_bytes(raw) != entry["sha256"]:
            raise KnowledgeError(
                "REVIEW_FILE_DRIFT",
                "review manifest identity differs from the linked file",
                exit_code=3,
                evidence_refs=[str(direct_path)],
                recoverable=True,
            )
        manifest.append({**copy.deepcopy(entry), "direct_path": str(direct_path.absolute())})
    return manifest
...
def review_candidate(
    ...
    *,
    ...
    """Validate a complete Candidate bundle and return its closed Chat projection."""
    ...
    _validated_finalizers(repo, candidate, candidate_path)
    validated_operations = _validated_operations(
        repo,
        candidate,
        candidate_path,
        known_secret_values=(),
    ...
    review, review_raw, review_path = _validated_review_bundle(
        repo,
        repo_id=repo_id,
        repo_root=repo_root,
        candidate_ref=candidate_ref,
    ...
    registry_root = repo_root.parents[1]
    manifest = _project_review_manifest(
        registry_root=registry_root,
        candidate_path=candidate_path,
        review=review,
    )

    # Close the projection-read window with a complete second validation pass.
    # This rechecks Candidate bytes, all sources/preimages, every postimage,
    # every supporting/automatic review file, review.json, and every direct link.
    candidate_after, candidate_path_after = _load_candidate(repo_root, candidate_ref)
    if candidate_path_after != candidate_path or candidate_after != candidate:
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "Candidate changed during review projection",
            exit_code=3,
            evidence_refs=[str(candidate_path)],
            recoverable=True,
        )
    _validated_finalizers(repo, candidate_after, candidate_path_after)
    validated_operations_after = _validated_operations(
        repo,
        candidate_after,
        candidate_path_after,
        known_secret_values=(),
    )
    _validate_stage_semantics(
        repo,
        str(candidate_after.get("stage", "")),
        str(candidate_after.get("work_id", "")),
        validated_operations_after,
        decision=str(candidate_after.get("decision", "")),
    )
    try:
        review_after, review_raw_after, review_path_after = _validated_review_bundle(
            repo,
            repo_id=repo_id,
            repo_root=repo_root,
            candidate_ref=candidate_ref,
            candidate=candidate_after,
            candidate_path=candidate_path_after,
            validated_operations=validated_operations_after,
            expected_review_sha256=review_sha256,
        )
    except KnowledgeError as exc:
        if exc.code != "REVIEW_BINDING_DRIFT":
            raise
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "human Gate review changed during review projection",
            exit_code=3,
            evidence_refs=[str(review_path)],
            recoverable=True,
        ) from exc
    manifest_after = _project_review_manifest(
        registry_root=registry_root,
        candidate_path=candidate_path_after,
        review=review_after,
    )
    candidate_final, candidate_path_final = _load_candidate(repo_root, candidate_ref)
    if candidate_path_final != candidate_path or candidate_final != candidate:
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "Candidate changed after linked-file projection",
            exit_code=3,
            evidence_refs=[str(candidate_path)],
            recoverable=True,
        )
    _validated_finalizers(repo, candidate_final, candidate_path_final)
    validated_operations_final = _validated_operations(
        repo,
        candidate_final,
        candidate_path_final,
        known_secret_values=(),
    )
    _validate_stage_semantics(
        repo,
        str(candidate_final.get("stage", "")),
        str(candidate_final.get("work_id", "")),
        validated_operations_final,
        decision=str(candidate_final.get("decision", "")),
    )
    try:
        review_final, review_raw_final, review_path_final = _validated_review_bundle(
            repo,
            repo_id=repo_id,
            repo_root=repo_root,
            candidate_ref=candidate_ref,
            candidate=candidate_final,
            candidate_path=candidate_path_final,
            validated_operations=validated_operations_final,
            expected_review_sha256=review_sha256,
        )
    except KnowledgeError as exc:
        if exc.code != "REVIEW_BINDING_DRIFT":
        ...
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "human Gate review changed after linked-file projection",
            exit_code=3,
            evidence_refs=[str(review_path)],
            recoverable=True,
        ) from exc
    if (
        review_path_after != review_path
        or review_path_final != review_path
        or review_raw_after != review_raw
        or review_raw_final != review_raw
        or review_after != review
        or review_final != review
        or manifest_after != manifest
    ):
        raise KnowledgeError(
            "REVIEW_MANIFEST_DRIFT",
            "complete review projection changed between validation passes",
            exit_code=3,
            evidence_refs=[str(review_path)],
            recoverable=True,
        )
    identity = copy.deepcopy(review_after["identity"])
    identity["review_sha256"] = review_sha256
    return {
        "schema": "human-gate-summary/v1",
        "gate": copy.deepcopy(review_after["gate"]),
        "summary": copy.deepcopy(review_after["summary"]),
        "risk_and_compatibility": copy.deepcopy(review_after["risk_and_compatibility"]),
        "validation": copy.deepcopy(review_after["validation"]),
        "review_bundle": {
            "review_ref": review_after["review_bundle"]["review_ref"],
            "review_sha256": review_sha256,
            "direct_path": str(review_path.absolute()),
            "manifest": manifest_after,
        },
        "identity": identity,
        "prompt": review_after["prompt"],
    }
...
def apply_candidate(
    ...
    *,
    ...
    candidate, candidate_path = _load_candidate(repo_root, candidate_ref)
    review_path = candidate_path.with_name("review.json")
    knowledge_governance._stable_registry_read(
        repo_root.parents[1],
        review_path,
        missing_code="LEGACY_RESEAL_REQUIRED",
        invalid_code="REVIEW_MANIFEST_DRIFT",
    ...
    if not isinstance(review_sha256, str) or re.fullmatch(r"[a-f0-9]{64}", review_sha256) is None:
        raise KnowledgeError(
            "REVIEW_BINDING_MISSING",
            "apply requires the exact review digest shown for this Candidate",
            exit_code=2,
            evidence_refs=[str(review_path)],
            recoverable=True,
        )
