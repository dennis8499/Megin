...
6. BUG delivery另要求`work_kind: bug`、primary bug ID、assessment JSON／Markdown與Requirements同一approval binding、Ready `bug_context`完全相符；Complete時delivery record、implementation run、review report與repository `bug-verification/v1`的path／hash／result四方一致。舊record缺`work_kind`仍視為standard。
