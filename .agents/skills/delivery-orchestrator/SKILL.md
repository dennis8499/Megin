...
description: 交付軟體變更的統一入口：先依任務大小分類，再建立或續接 Work ID 綁定的 workspace，依目前 phase 路由需求探索、技術規劃、實作執行與 fresh review。適用於新功能、已完成唯讀分診的修錯、實質重構及介面／資料／依賴變更；純解說、診斷、審查、plan-only、格式與微小文字修改不適用。新 portable run 使用 `delivery-run/v2`；沒有 v2 state 的既有工作維持 v1 相容流程。
---
...
| 疑似 BUG 但尚無 assessment | 先用 `bug-diagnosis` 唯讀分診；此時不建立 workspace |
| `not-a-bug` | 期望行為改變時走 standard requirements，否則結束且不建立 run |
| 純解說、診斷、唯讀審查、plan-only | 對應一般／階段工作流 |
| 格式或微小文字修改 | 直接處理，不建立 delivery run |

完成條件：請求唯一落在一列；不適用時沒有 registry、branch 或 workspace mutation。

## 不變量

- New work 的 base 是 strict-clean attached primary；resume 只接受可驗證 record。
- v1 helper 的唯一自動 Git mutation 是建立隔離 worktree／branch；v1 嚴禁 stash、reset、clean、stage、commit、push、merge、deploy、delete 或 cleanup。v2 只在核准的 workspace mode 建立目前目錄功能 branch 或 worktree，`unstaged` finish 不得 stage、commit、push 或建立 PR。
...
- v2 的 task class、approval policy、writer assignment、knowledge scope、workspace 與 publication destination 都由持久化 state 的 digest 綁定；分類及核准前不建立產品 workspace 或 branch。
- v2 同一 workspace 同一時間最多一名取得授權的 implementation writer。writer 可以是 implementation subagent；不得平行寫入，Reviewer 必須是另一個 fresh、唯讀 session。
...
核准後新 v2 工作預設從核准的 base branch 在目前 checkout 建立 `feat/<work-id>`；只有明示
`workspace_mode=worktree` 才建立 Work ID 專用 worktree。接著自動 dispatch 下一個已授權 phase。
...
完成條件：schema-valid record 的 repo、workspace、branch、base 與 registry binding 全相符；new generation 另為 ready。後續 repository 命令以該 workspace 為 cwd。
...
交付回報 work_id、generation、workspace／branch、phase／status、current refs、next action 與 record path；Complete 另含 fresh verdict、交付 snapshot 與未提交 diff 或 commit identity。

完成條件：v1 terminal ordering、schema 與 append-only semantics 通過；workspace 保留且沒有 terminal Git／發布動作。v2 另須由 [v2 派工、審查與交付收尾契約](../implementation-execution/references/v2-dispatch-and-finish.md) 完成 approved-scope knowledge review 與 `finish` handoff；預設為 `delivered_unstaged`，只有核准 `commit`／`draft-pr` mode 才可 commit、push 或建立 draft PR，且不得 merge、deploy 或 cleanup。
