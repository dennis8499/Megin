...
description: Execute an approved Megin v2 dispatch package in the bound current checkout or worktree with one authorized writer, TDD evidence, bounded fixes, and a fresh read-only review handoff.
---
...
write. Never create a second writer for the same repository, and never stage or commit while the
approved finish mode is `unstaged`.
