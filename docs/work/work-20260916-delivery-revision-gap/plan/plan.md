...
- 日期：2026-09-17
- 來源規格：`docs/work/work-20260916-delivery-revision-gap/requirements.md`
...
- Primary／handoff：`docs/work/work-20260916-delivery-revision-gap/plan/plan.md`／`docs/work/work-20260916-delivery-revision-gap/plan/handoff.json`
...
| CON-004 | v1 首版命名、既有 phase gate 與 Windows／Linux release evidence 邊界維持不變。 | SRC-001、SRC-005、SRC-006 |

## 2. 證據與變更影響

| SRC ID | Kind／location／revision | 事實 | Plan refs | 直接 WP refs |
|---|---|---|---|---|
| SRC-001 | spec／`docs/work/work-20260916-delivery-revision-gap/requirements.md`／requirements-candidate-2 | Required：FR-001..005、NFR-001、TR-001 與 AC-001..007 定義 high-water、collision、原子性、首版相容及同一 Requirements Gate。 | CON-001..004、TD-001、BDD-001、BDD-002、TEST-001、TEST-002 | WP-001 |
| SRC-002 | bug／`docs/bugs/bug-delivery-plan-revision-gap/assessment-1.json`／1 | Observed：plan-2 後 generation-2 缺席較舊 `plan/`，有效 plan-3 transition 回傳 `INVALID_REVISION`；root cause 已確認。 | CON-001、CON-003、TD-001、BDD-001、TEST-001 | WP-001 |
...
| SRC-006 | project／`.github/workflows/knowledge-portability.yml`／026a491ead8efe81797095e485cc3138824e6ec2 | Observed：既有 portability workflow 以 Python 3.13、Linux／Windows matrix 執行 quick、plugin 與 full suite；跨平台證據屬 release obligation。 | CON-004、CMD-CI-001、BDD-002、TEST-002 | WP-001 |
...
| Verification boundary | 現有 repo 使用 standard-library `unittest` 與 portability workflow。 | 以同一 isolated fixture 走真實 `workspace.transition_record`；focused cases 先 red，再 minimal green；related／full／release commands 保留完整 inventory。 | SRC-004、SRC-006／TD-002 |
...
- Assessment JSON：`docs/bugs/bug-delivery-plan-revision-gap/assessment-1.json`，SHA-256 `050416643def2603de0c1b4e051446d2179f5ee2f6dc2680fe712fdfebe34118`；Markdown：`docs/bugs/bug-delivery-plan-revision-gap/assessment-1.md`，SHA-256 `1b41d452c40b5532f5a2b5d2cc16f23dd4c04beed9a190f9f7826996a9c234a6`。
...
| CMD-CI-001 | ci | Proposed | 讀取既有 portability workflow 的 hosted Linux／Windows run，確認兩平台報告均通過；不在本 Work 自動 dispatch 或 push。 |

`validation-plan/v1`：日常 target 為 Windows 11、Python 3.13+、Git 026a491e baseline、PowerShell 7 與 ripgrep；local obligations 綁定 `CMD-BUG-REPRO-001`、BDD/TDD focused、related、full build/test 與 governance。release obligations 另要求既有 workflow 的 hosted Windows／Linux reports；單一本機平台不冒充雙平台證據。Final review 只能引用本 Work ID 的 implementation Outcome、fresh review、Knowledge promotion 與 terminal evidence；不得重用未列於 terminal-only policy 的可執行輸入。
...
- 要求／結果／impact：FR-001..005、NFR-001、TR-001、AC-001..AC-007、CON-001..004。
- Blocked by：None。
...
- Commands／完成證據：`CMD-BUG-REPRO-001`、`CMD-BDD-DISCOVERY-001`、`CMD-BDD-FOCUSED-001`、`CMD-BDD-FOCUSED-002`、`CMD-TDD-FOCUSED-001`、`CMD-TDD-FOCUSED-002`、`CMD-BDD-FULL-001`、`CMD-RELATED-001`、`CMD-BUILD-FULL-001`、`CMD-TEST-FULL-001`、`CMD-GOVERNANCE-001`、`CMD-CI-001`；保存 red→green、完整 inventory、record／path byte snapshots 與獨立 fresh review。
...
| RISK-005 | 本地 Windows 通過被誤報為跨平台完成 | Linux compatibility 未證明 | 將 hosted workflow 的 Linux／Windows reports 列為 release obligation；缺 CI 時保留待驗證狀態 | WP-001／Platform reviewer |
...
| SRC-006／Windows／Linux release obligation | TD-002／MOD-003／SEAM-003 | BDD-002 | TEST-002 | WP-001 | CMD-CI-001 |
...
| Path | Role | 建立理由 | 權威內容 |
|---|---|---|---|
| `docs/work/work-20260916-delivery-revision-gap/plan/plan.md` | primary | 主線方案、設計、測試與工作包索引 | 本文件 |
| `docs/work/work-20260916-delivery-revision-gap/plan/handoff.json` | handoff | Implementation consumer 的 `ready-plan/v1` contract | 完整 machine contract、commands、sources、DAG、validation |
...
- 品質門檻：來源、設計、測試、原子性、相容性、release obligations 與追溯矩陣均通過；本 Candidate 尚待 Plan Gate 核准。
