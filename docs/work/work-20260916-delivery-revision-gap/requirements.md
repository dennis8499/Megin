# 需求分析：跨世代的 Delivery 修訂版配置

- 文件狀態：Ready（本 Candidate 尚待 Requirements Gate 核准；核准前不生效）
- 日期：2026-09-16
- 文件範圍：修正 repository-local Delivery v1 在 generation 重建後配置下一個 Requirements／Plan 修訂版時的錯誤拒絕。
- 需求來源：使用者授權的獨立 bug 修復、已確認的 BUG assessment、現有 Delivery v1 記錄與核准契約。
- 確認者：使用者（本版待精確 Candidate 核准）
...
- 完成結果：有效新 revision 成為 current ref；失敗時 record、current ref 及既有路徑 bytes 不變。
...
|---|---|---|---|---|---|
| KPI-001 | BG-001 | Windows 與 Linux 上，AC-001、002、004、006 成功；AC-003、005、007 仍 fail-closed | AC-001 目前 0 通過 | 100% | 本 Work 完整驗證期間；隔離 fixtures、Delivery transition tests、CI reports |

## 8. 追溯矩陣

| 業務成果 | 旅程 | 需求 | 驗收情境 | 成功指標 |
|---|---|---|---|---|
| BG-001 | J-001 | FR-001–FR-004、NFR-001、TR-001 | AC-001–AC-006 | KPI-001 |
...
- DEP-002：regression、fresh review 與跨平台驗證命令由 Planning 依 repository tests／CI evidence 確定。
...
| 品質屬性 | 已確認 | NFR-001 與 Windows／Linux KPI |
...
- Assessment：docs/bugs/bug-delivery-plan-revision-gap/assessment-1.md（SHA-256 1b41d452c40b5532f5a2b5d2cc16f23dd4c04beed9a190f9f7826996a9c234a6）；docs/bugs/bug-delivery-plan-revision-gap/assessment-1.json（SHA-256 050416643def2603de0c1b4e051446d2179f5ee2f6dc2680fe712fdfebe34118）
